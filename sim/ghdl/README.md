...
| `tb_sdram.vhd` | SDRAM controller bring-up. Drives `sdram_controller`'s Wishbone port against `sim/sdram_model.vhd`: power-on init, single write/read, consecutive words (catches address overlap), and a second row and bank. Every access is bounded by a timeout, because neither `bus_interconnect` nor `MEM_Stage` has a watchdog — on hardware a controller that fails to ack freezes the CPU with no diagnostic. |
